...
def Kc_to_Kc_prime(Kc, L, red=False):
    ...
    """
    assert len(Kc) == Ar_KEY_LEN and isinstance(Kc, bytearray)
    assert isinstance(L, int) and (0 < L < 17)
    ...
    if L == 16:
        ...
        assert len(Kc_prime) == Ar_KEY_LEN and isinstance(Kc_prime, bytearray)
