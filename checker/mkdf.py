...
def mac(LK, EN_NONCE, BTADDR):
    ...
    assert isinstance(EN_NONCE, bytearray) and len(EN_NONCE) == 16
...
def mkdf(LK, AU_RAND, EN_NONCE_C, EN_NONCE_P, BTADDR, ENTROPY):
    ...
    assert isinstance(EN_NONCE_C, bytearray) and len(EN_NONCE_C) == 16
    assert isinstance(EN_NONCE_P, bytearray) and len(EN_NONCE_P) == 16
    assert isinstance(BTADDR, bytearray) and len(BTADDR) == 6
    assert isinstance(ENTROPY, int) and 0 < ENTROPY <= 16
